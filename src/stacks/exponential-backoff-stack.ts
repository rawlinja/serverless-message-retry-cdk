...
class ExponentialBackoffStack extends Stack {
    constructor(scope: Construct, id: string, props: ExponentialBackoffStackProps) {
        ...
        this.build({ handler: props.handler, tableArn: props.tableArn, tableStreamArn: props.tableStreamArn })
    }
   ...
   //// Build delete trigger
    build(buildProps: { handler: string, tableArn: string, tableStreamArn: string }) {
        ...
        deleteTrigger.addToRolePolicy(new iam.PolicyStatement({
        ...
        deleteTrigger.addEventSource(new lambdaEventSources.DynamoEventSource(table, {
